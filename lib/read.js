...
var build = function (data, options, dirname, cb){
  ...
  if (options.include){
    ...
    var include = function (value){
      ...
      read (p, options, function (error, included){
        ...
        if (!remainingIncluded) cb (null, options.namespaces ? n : o, comments);
      });
    };
  }
  
  if (!data){
    if (cb) return cb (null, o, comments);
    return o;
  }
  ...
  if (options.reviver){
    if (options.sections){
      line = function (key, value, comment){
        if (options.include && key === INCLUDE_KEY) return include (value);
        if (comment) comments[key] = comment.trim();
  ...
  if (options.variables){
    handlers.line = function (key, value, comment){
      expand (options.namespaces ? n : o, key, options, function (error, key){
        ...
        expand (options.namespaces ? n : o, value, options,
          ...
          line (key, cast (value || null), comment);
        });
      });
    };
  ...
  if (cb) return cb (null, options.namespaces ? n : o, comments);
